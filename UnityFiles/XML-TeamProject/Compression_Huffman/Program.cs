...
namespace Huffman_Encoding
{
    public class PriorityQueue<T> where T : IComparable
    {
        ...
        public virtual T Pop()
        {
            if (LstHeap.Count == 0)
            {
                throw new IndexOutOfRangeException("Error: Trying to Pop an empty priority queue");
            }
        ...
        protected bool RSonExists(int i)
        {return RChildIndex(i) < LstHeap.Count;}
 
        protected bool LSonExists(int i)
        ...
        protected void DownHeap(int i)
        {
            while (i >= 0)
            {
                ...
                if (RSonExists(i) && Right(i).CompareTo(ArrayVal(i)) > 0)
                {
                    iContinue = Left(i).CompareTo(Right(i)) < 0 ? RChildIndex(i) : LChildIndex(i);
                }
                else if (LSonExists(i) && Left(i).CompareTo(ArrayVal(i)) > 0)
                {
    ...
    public class Huffman<T> where T : IComparable
    {
        ...
        public void Encode(T value, List<int> encoding)
        {
            ...
            {
                throw new ArgumentException("Error: Unallowed value inside Encode");
            }
        ...
        public T Decode(List<int> bitString, ref int position)
        {
            ...
            while (!nodeCur.Is_a_Leaf)
            {
                ...
                {
                    throw new ArgumentException("Error: Unallowed bitstring inside the Decode");
                }
